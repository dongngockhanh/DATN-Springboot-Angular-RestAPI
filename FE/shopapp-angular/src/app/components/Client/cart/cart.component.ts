...
export class CartComponent {
  ...
  removeSelectedProduct() {
    ...
    this.updateSelectedProductCount();
  }  // sự kiện click vào nút mua ngay(lấy id sản phẩm được chọn và lấy dữ liệu đó chuyển sang trang khác để xử lý tiếp)
  buyNow() {
    debugger
    let selectedProductIds = this.products.filter(product => product.selected).map(product => product.id);
    // chuyển trang và truyền danh sách id sản phẩm được chọn
    this.router.navigate(['/order'], { queryParams: { ids: selectedProductIds.join(',') } });
  }
}
